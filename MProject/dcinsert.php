...
while ($row = mysqli_fetch_array($result)) {
	if (empty($row[1])) {
		$temp=$row[0];
		$sql = "UPDATE deluxe SET name='$name' ,contact='$contact', checkin = '$checkin' , checkout = '$checkout' , numguest = '$numguest', payment = '$payment' , cardnum = '$cardnum' , valid = '$valid' , expdate = '$expdate' , cardname = '$cardname' WHERE num='$temp'";
		if ( $mysqli->query($sql) ){
			break;
		}
	}
