...
while ($row = mysqli_fetch_array($result)) {
	if (empty($row[1])) {
		$temp=$row[0];
		$sql = "UPDATE standard SET firstname='$firstname' , lastname='$lastname' , checkin = '$checkin' , checkout = '$checkout' , numguest = '$numguest', payment = '$payment', cardnum = '$none' , valid = '$none' , expdate = '$none' , cardname = '$none', contact='$contact', status='$status' WHERE num='$temp'";
